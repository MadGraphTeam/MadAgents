...
|---|---|---|
| **Consultant / probe** | `.madagents/wiki/consultants/<your-agent-name>/` | `.claude/agent-memory/<your-agent-name>/MEMORY.md` |
| **Lead** | `.madagents/wiki/lead/` (flat, no subdirs) | `.claude/lead-memory/MEMORY.md` |

`<your-agent-name>` is your literal agent filename without `.md` (e.g. `ma-chain-decay-consultant`), suffix and all. The slate path is **absolute** — never write a slate under a relative path, and never under `.madagents/wiki/` (a slate written there is never auto-loaded and is silently inert).
