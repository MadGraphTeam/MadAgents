...
A consultant's slate is read only from `.claude/agent-memory/<agent-name>/MEMORY.md`; a slate written under the wiki (`.madagents/wiki/`) is never loaded, so its content is silently inert. After aggregating, the lead audits the writes:

1. `find .madagents/wiki -name MEMORY.md ! -empty` returns nothing. A hit is a consultant that wrote its slate relative to its wiki subtree instead of the project-root path.
2. Each consultant dispatched this run has a non-empty slate at `.claude/agent-memory/<agent-name>/MEMORY.md`.
3. Consultant pages sit under `.madagents/wiki/consultants/<agent-name>/`, lead pages under `.madagents/wiki/lead/`.

For any populated slate found under `.madagents/wiki`: move it to the project-root path when that copy is missing, otherwise delete the stray; then re-dispatch the owning consultant to write its slate to the project-root path. Report what was checked and corrected.
