...
- `codex-mechanics.md` — what Codex can and cannot do, so a recommendation is buildable.
...
Before anything else, archive the current harness so the whole operation is revertable to the exact pre-doctor state:
    ...
    cp -r "$root/.codex" "$root/.agents" "$root/prompts" "$root/.madagents/harness-archive/$ts/"

The harness is three trees here, not one: `.codex/agents/` (the roster — and, inside each role file, that agent's slate), `.agents/skills/` (the workflows), and `prompts/` (the lead's append). Archive all three or the revert is partial.
...
Apply the approved change to the harness trees. When editing an existing agent card, edit the **card half** of its `developer_instructions` in place — do not overwrite its `description`, which may carry a learned routing refinement, and **never touch the marked slate region**, which is that agent's own learned tier and not yours to rewrite. A card edit is a TOML edit: keep the file valid, or the agent disappears from the roster with no error.
...
Two Codex-specific assertions belong in the same pass, because both fail silently:
...
for p in sorted(pathlib.Path(".codex/agents").glob("*.toml")):
    ...
    assert d.get("name") == p.stem, p
    assert d.get("description") and d.get("developer_instructions"), p
print("roster parses")
PY
```

- **Every role file still parses**, and its `name` still matches its filename. A malformed role is dropped from the roster outright, so a broken card looks exactly like a consultant that was never there.
- **No slate region was disturbed.** Diff the marked regions against the snapshot: they should be byte-identical, because this skill edits cards and the agents own their slates.

**A fix in one tier is not a fix.** If the discipline you changed is also stated in the learned tier (a role's slate region, `.madagents/memory/lead/MEMORY.md`, `.madagents/wiki/`), that tier now contradicts the surface **in the same reader's context**. Correct it in the same pass — through the owning agent, per single-writer — or tell the user plainly that you did not.

## 6. Panel review

Dispatch the `reviewers/` briefs as general-purpose agents on the applied change — route by relevance (a runtime-neutral prose tweak need not pay `codex-alignment`), with the full panel as the minimum gate for anything non-trivial. Each returns a verdict. (Keep dispatch with you, the main session — the panel is a flat fan-out the lead drives.)
