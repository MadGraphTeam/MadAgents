...
7. **Verify placement.** A consultant's slate is auto-loaded only from the marked region of its own `.codex/agents/<name>.toml`, written through `ma-wiki-write`'s `write_slate.py`; the lead's only from `.madagents/memory/lead/MEMORY.md`. A slate written as a loose `MEMORY.md` anywhere in the tree is never loaded and is silently inert. After writing, audit:

   ```bash
   find . -name MEMORY.md ! -path './.madagents/memory/lead/*' ! -empty   # expect no output
   for a in <agents hardened this run>; do
     python3 .agents/skills/ma-wiki-write/scripts/write_slate.py --agent "$a" --show
   done
   ```

   The first returns nothing; the second returns a non-empty slate for each agent hardened this run. A stranded `MEMORY.md` means an agent wrote a file instead of calling the script — fold its content into the real slate (or delete it if the real one is already current) and re-dispatch the owning agent to write through the script.
