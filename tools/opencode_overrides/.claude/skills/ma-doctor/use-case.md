...
An **add-on a user drops into an existing opencode session** to give it MadGraph competence — not a standalone app. It layers onto the user's setup and should add to any existing agent system without taking it over. It is implemented entirely through the opencode configuration surface (the agent roster and its card bodies, skills, the lead's instructions, the permission block); a capability that exists only in some other CLI or SDK is out of scope.
...
The bar is **bare opencode**. The user could just ask their own session to do the MadGraph work, so this must be **better than bare opencode** to be worth adding: more accurate, and worth its token and latency overhead. Every scaffold earns its keep against that baseline. The core tension: the rich multi-agent setup must stay token-frugal enough to beat bare opencode.
...
The setup should perform well even with weaker (e.g. local open-source) models — "warm and well trained at start" yet evolving with use. The trained state (memory + wiki) carries performance when the model is weaker, so a thinning justified only by "the model is strong" is a present-tense call, not future-proof. **On this provider that is the present tense, not the future**: opencode is how this system runs against a self-hosted model, so the reader of a change made here is more likely than not to be a smaller open-weights model. Weigh every cut accordingly — the scaffolding a frontier model could infer is exactly what a weaker one cannot.
