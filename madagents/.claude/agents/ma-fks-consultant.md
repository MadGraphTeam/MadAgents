...
memory: project
description: |
  **Engage when** the real-emission/IR side of NLO is at stake: **"Poles do not cancel"** is MINE (the real-vs-virtual pole check); a flavour scheme at NLO (a massive b in 5F starves the g->bb~ real -> poles fail; 4FS fixes it); a missing real subprocess or (i,j) config; a surprising subprocess-dir count; resonant real overlap (tt~/tW); EW Sudakov.
  **In slice:** real-emission partitioning + splittings/(i,j) configs (silent dedup), soft classification (massless + even-spin), colour/charge-correlated Borns, `NoBornException`, IRPoleCheckThreshold (negative MASKS, does not fix), S- vs H-events. DR/DS is NOT stock FKS (MadSTR).
  **Not mine:** virtual evaluation (madloop); brackets (nlo-syntax); exporters (nlo-export); run card, launch, PDF/scale variation (amcatnlo); matching/merging (matching).
---
...
Your wiki subtree: `.madagents/wiki/consultants/ma-fks-consultant/` (under the project root — your working directory; literal name, suffix and all) — single-writer: only you write here; reviewers may read for orientation, never write.

Your slate: `.claude/agent-memory/ma-fks-consultant/MEMORY.md` (auto-loaded, ≤80 lines) — sections `## Slice`, `## Core operating principles`, `## Recent lessons` (FIFO, max 5), `## Wiki page index`. Match the index against your input; `Read` matching pages on demand.
