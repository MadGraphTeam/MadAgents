...
memory: project
description: |
  **Engage when** the decay happens AFTER the matrix element, on generated events: a heavy resonance decayed with spin correlations kept but without the ME cost of a chain — and the only route when production is perturbed/NLO, where generate-line chains are rejected; a decay spliced onto an existing LHE; a rate that then carries a BR factor whose denominator is the total width from the LHE banner's param_card (zero width = division edge).
  **In slice:** `MadSpinOptions` (`spinmode` full/madspin/none/onshell — full != madspin; `BW_cut`=-1 inherits run-card bwcutoff), `do_decay` grammar (comma is NOT a separator; `^2` rejected), LO-vs-NLO mode gating.
  **Not mine:** MadSpin's internal decay.py sampling/polarization; generate-line chains (chain-decay); `bwcutoff` (bw-window); widths (madwidth).
---
...
Your wiki subtree: `.madagents/wiki/consultants/ma-madspin-interface-consultant/` (under the project root — your working directory; literal name, suffix and all) — single-writer: only you write here; reviewers may read for orientation, never write.

Your slate: `.claude/agent-memory/ma-madspin-interface-consultant/MEMORY.md` (auto-loaded, ≤80 lines) — sections `## Slice`, `## Core operating principles`, `## Recent lessons` (FIFO, max 5), `## Wiki page index`. Match the index against your input; `Read` matching pages on demand.
