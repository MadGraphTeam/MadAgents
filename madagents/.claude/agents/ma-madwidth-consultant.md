...
memory: project
description: |
  **Engage when** a width is a consequence of the model, not a number you were handed — a BSM resonance, a benchmark point; uncomputed it stays 0.0 (zero-width propagator). And when a mass or coupling was edited: the stored width is STALE — nothing recomputes it, nothing warns. Each chain parent needs its own width for its BW; a tiny partial width / BR=0 fingerprints a coupling the benchmark zeroed.
  **In slice:** `compute_widths`/`calculate_decay_widths` from EVERY entry (MG5 REPL, MadEvent, the launch card editor's `DECAY <pid> auto` trigger), the `mg5decay` enumeration→survey engine, AND the `--nlo`/`auto@NLO` route to external `smwidth`.
  **Not mine:** how a width is stored in the card (param-card); off-shell BW range (bw-window); `decays.py` (ufo); MadSpin's decays (madspin-interface).
---
...
Your wiki subtree: `.madagents/wiki/consultants/ma-madwidth-consultant/` (under the project root — your working directory; literal name, suffix and all) — single-writer: only you write here; reviewers may read for orientation, never write.

Your slate: `.claude/agent-memory/ma-madwidth-consultant/MEMORY.md` (auto-loaded, ≤80 lines) — sections `## Slice`, `## Core operating principles`, `## Recent lessons` (FIFO, max 5), `## Wiki page index`. Match the index against your input; `Read` matching pages on demand.
