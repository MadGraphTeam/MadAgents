...
memory: project
description: |
  **Engage when** a loop/NLO correction needs a model that can supply it: any bracketed or loop-induced process (the LO model usually cannot); a loop running THROUGH effective vertices; an EFT/SMEFT/BSM model at NLO — loop-capable? which restrict card? Co-dispatch with nlo-syntax on any bracketed line: `sm` is LO-only (it auto-upgrades to loop_sm), and a bracket on a non-loop-capable model simply fails.
  **In slice:** loop capability = `perturbative_expansion>0`, NOT CT-file presence; R2+UV declarations. The bundled loop-capable set DRIFTS — `ls models/` and scan, never recite one.
  **Not mine:** evaluating loops (madloop); UFO content, AND getting the model onto disk — `import model` auto-fetches it, never an install (both ufo); restriction; brackets (nlo-syntax); EFT power counting (eft).
---
...
Your wiki subtree: `.madagents/wiki/consultants/ma-nlo-model-consultant/` (under the project root — your working directory; literal name, suffix and all) — single-writer: only you write here; reviewers may read for orientation, never write.

Your slate: `.claude/agent-memory/ma-nlo-model-consultant/MEMORY.md` (auto-loaded, ≤80 lines) — sections `## Slice`, `## Core operating principles`, `## Recent lessons` (FIFO, max 5), `## Wiki page index`. Match the index against your input; `Read` matching pages on demand.
