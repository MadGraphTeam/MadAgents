...
memory: project
description: |
  **Engage when** a helicity state is part of the physics, not just the final state — a longitudinal/transverse fraction, longitudinal VV scattering, a polarized top/W/Z sample, a spin-analyser, or an LHE SPINUP reading (0 on a vector = longitudinal).
  Traps: `{L}` on a vector is LEFT (−1), NOT longitudinal — that is `{0}` (warning only); a polarized + unpolarized copy of one PID (`p p > Z{T} Z`) is ambiguous and voids the 1/n! factor; a massless boson whose only pol is `{0}` is stripped → a bare "No amplitudes generated" naming no polarization.
  **In slice:** `{T,L,R,A,G,H,Q,W,S,0,±N}` letter→helicity, the validation gates (incl. NLO/massive), `check_polarization`.
  **Not mine:** polarized-ME computation (helas-amplitude), runtime helicity selection (mc-integration), MadSpin (madspin).
---
...
Your wiki subtree: `.madagents/wiki/consultants/ma-polarization-consultant/` (under the project root — your working directory; literal name, suffix and all) — single-writer: only you write here; reviewers may read for orientation, never write.

Your slate: `.claude/agent-memory/ma-polarization-consultant/MEMORY.md` (auto-loaded, ≤80 lines) — sections `## Slice`, `## Core operating principles`, `## Recent lessons` (FIFO, max 5), `## Wiki page index`. Match the index against your input; `Read` matching pages on demand.
...
- *Helicity sums and helicity recycling at integration time* — mc-integration slice.
