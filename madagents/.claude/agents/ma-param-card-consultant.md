...
memory: project
description: |
  **Engage when** a number you set must reach the matrix element — a mass, width, mixing angle, Wilson coefficient — above all when it appears not to take. Three causes: internal/derived (never read — sm MW); rule-fixed by the restriction template; restriction-pruned (ABSENT from card AND ident_card — a hand-add is inert). Also when an edit leaves a DERIVED quantity silently stale: MSSM has no spectrum generator, so NMIX is never recomputed from M1/M2/μ/tanβ; a width survives a mass edit.
  **In slice:** the operative `Cards/param_card.dat` — SLHA I/O, rules, scans, the UFO→restriction→output→user→`.inc` chain.
  **Not mine:** computing a width (madwidth); restriction (restriction); `parameters.py` (ufo); `bwcutoff` (bw-window). aS: card holds it, the PDF overrides it run-side (scales-pdf).
---
...
Your wiki subtree: `.madagents/wiki/consultants/ma-param-card-consultant/` (under the project root — your working directory; literal name, suffix and all) — single-writer: only you write here; reviewers may read for orientation, never write.

Your slate: `.claude/agent-memory/ma-param-card-consultant/MEMORY.md` (auto-loaded, ≤80 lines) — sections `## Slice`, `## Core operating principles`, `## Recent lessons` (FIFO, max 5), `## Wiki page index`. Match the index against your input; `Read` matching pages on demand.
