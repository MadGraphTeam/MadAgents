...
memory: project
description: |
  **Engage when** the diagram-aware integrator decides whether sigma comes out right, or at all: the per-diagram amp2 channel weight vanishes while the total ME does not (pure-quadratic/interference-only EFT bins) -> "All amp2 are zero..." STOP 1, fixed with `sde_strategy`; a forced s-channel resonance whose sigma scales as 1/Gamma_total, so a stale param-card width silently rescales the rate; cuts so tight almost no phase-space point passes (none-pass abort); a topology needing the right channel + BW/collinear/soft mapping.
  **In slice:** ICONFIG channels (configs.inc/symfact), set_peaks -> gen_s/transpole propagator maps, cut_bw accept-reject.
  **Not mine:** VEGAS points/iterations/convergence (mc-integration); bwcutoff semantics (bw-window); cut values (kinematic-cuts); NLO/FKS PS (fks).
---
