...
memory: project
description: |
  **Engage when** what the GENERATED ME computes is at issue, not which diagrams exist — a σ off by an exact factor (spin, initial-colour and identical-particle 1/n! all fold into one IDEN); Majorana fermions (fermion-flow clash, signs); which helicity combinations the ME enumerates at all.
  The emitted `matrix_*.f`/JAMP is writer-, flag- and gauge-selected — READ it, never predict from source defaults.
  **In slice:** WAVEFUNCTION recycling (identical-wavefunction dedup) + the helicity SUM the ME builds, JAMP + s/t-channel configs.
  **Not mine:** enumeration (diagram-enumeration); colour (color-decomposition); loop HELAS (madloop); the EMITTED HELAS routines + the stock HELAS library (aloha); HELICITY recycling — generated/disabled (output), good_hels/limhel at RUN time (mc-integration).
---
...
Your wiki subtree: `.madagents/wiki/consultants/ma-helas-amplitude-consultant/` (under the project root — your working directory; literal name, suffix and all) — single-writer: only you write here; reviewers may read for orientation, never write.

Your slate: `.claude/agent-memory/ma-helas-amplitude-consultant/MEMORY.md` (auto-loaded, ≤80 lines) — sections `## Slice`, `## Core operating principles`, `## Recent lessons` (FIFO, max 5), `## Wiki page index`. Match the index against your input; `Read` matching pages on demand.
...
- *Per-event helicity MC selection at run time* — mc-integration slice; the recycler runs at output, the runtime helicity choice is separate.
