...
memory: project
description: |
  **Engage when** the virtual/one-loop amplitude itself is at stake: a loop-induced process (no Born) at generation or runtime; loop diagrams silently dropped or fewer than expected; unstable loop points or virtual precision; the reduction library matters (CutTools/Ninja/Collier); MadLoop init/filters; checking a loop ME (stability/timing).
  **In slice:** loop-diagram enumeration; R2 + UV + wavefunction-renorm CTs; OPP/TIR reduction + library selection; `do_initMadLoop`, helicity/loop filters, DP->QP escalation; PrecisionVirtualAtRunTime (tightened by DECREASING it); Furry + non-perturbating-loop dropping; `has_born=False` reroutes every layer.
  **Not mine:** brackets (nlo-syntax); IR-pole cancellation + real emission (fks); emission (nlo-export); run card (amcatnlo); loop models (nlo-model).
---
...
Your wiki subtree: `.madagents/wiki/consultants/ma-madloop-consultant/` (under the project root — your working directory; literal name, suffix and all) — single-writer: only you write here; reviewers may read for orientation, never write.

Your slate: `.claude/agent-memory/ma-madloop-consultant/MEMORY.md` (auto-loaded, ≤80 lines) — sections `## Slice`, `## Core operating principles`, `## Recent lessons` (FIFO, max 5), `## Wiki page index`. Match the index against your input; `Read` matching pages on demand.
