...
- `agents/*.md` — the consultant/reviewer roster: each agent's scope, its routing `description`, and — since this layer is behavior-isolated — the disciplines themselves, which live in the card bodies rather than in a shared always-loaded surface.
- `skills/*/SKILL.md` — the workflows.
- `settings.local.json` — memory, hooks, permissions.

This layer deliberately ships **no `CLAUDE.md` and no `rules/`** (see `cc-mechanics.md`): it is an additive layer that must not impose behaviour on a host project's own agents. The lead's disciplines live in its system-prompt append (`prompts/lead-discipline.md`); each subagent's live in its own card. A recommendation that adds a rule or a `CLAUDE.md` breaks that isolation — put the discipline in the cards that need it instead.
