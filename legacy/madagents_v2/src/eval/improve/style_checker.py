"""Style check harness: verify documentation formatting and consistency.

Graph shape::

    START → check_style → json_validate ─(valid)──→ END
                  ↑             │
                  └──(retry)────┘
"""
...
def build_style_graph(
    ...
    *,
    ...
    async def check_style_node(state: StyleState) -> dict:
        if session is None:
            return {"done": True}
        ...
        if attempt == 0:
            prompt = (
                _load_template("style.md", prompts_dir)
    ...
    def json_validate_node(state: StyleState) -> dict:
        if session is None:
        ...
        print(f"  Style validation failed: {'; '.join(validation.errors)}")
    ...
    def route(state: StyleState) -> str:
        return "valid" if state.get("done", False) else "retry"

    graph = StateGraph(StyleState)
    graph.add_node("check_style", check_style_node)
    graph.add_node("json_validate", json_validate_node)
    graph.add_edge(START, "check_style")
    graph.add_edge("check_style", "json_validate")
    graph.add_conditional_edges("json_validate", route, {"retry": "check_style", "valid": END})
...
async def run_style_check(
    ...
    *,
    ...
    """Run style check. Returns {"passed": bool, "issues": [...]}."""
    _prompts_dir = Path(prompts_dir) if prompts_dir else None
    print(f"\n  Checking style...")

    graph = build_style_graph(
