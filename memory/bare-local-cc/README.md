# `bare-local-cc` — harness know-how only, for self-hosted models

No MadGraph knowledge at all: an empty learned tier, plus the single principle on how to operate
Claude Code that [`pretrained-local-cc`](../pretrained-local-cc) adds to
[`pretrained`](../pretrained).

```bash
./madrun.sh --new --memory bare-local-cc
```
...
| Tree | What it holds |
| --- | --- |
| `.claude/lead-memory/MEMORY.md` | One section, `## Core operating principles`, with one bullet: how the lead waits for a dispatched subagent — end the turn, no tool call, and the harness re-invokes with the complete result. Byte-identical to the bullet `pretrained-local-cc` carries. |
...
[`pretrained-local-cc`](../pretrained-local-cc) in everything except the MadGraph knowledge**. Run the two
